...
const SpaceshipDetails = (props: ISpaceship) => {
  ...
  const renderedPilots =
    pilots &&
    pilots.map((pilot: string) => {
      return <ResourceValue key={pilot}>{pilot}</ResourceValue>;
    });

  const spaceshipDetails = { ...rest, renderedPilots, renderedFilms };

  const renderedSpaceship = mappedSpaceships(spaceshipDetails).map(
  ...
  return (
    <React.Fragment>
      ...
      <ResourceGrid container direction='row' justify='flex-start'>
        <ResourceList>{renderedSpaceship}</ResourceList>
      </ResourceGrid>
    </React.Fragment>
  );
};
