      ...
      <Grid
        ...
        <Grid container item sm={12} md={6}>
          {!!spaceships.length ? (
            <ResourcesList
              ...
              onResourceSelect={(spaceship: ISpaceship) =>
                setSelectedResource(spaceship)
              }
            />
          ) : (
        ...
        <Grid container item sm={12} md={6}>
          {!!Object.keys(selectedResource).length && (
            <SpaceshipDetails {...selectedResource} />
          )}
        </Grid>
      </Grid>
    </Container>
...
const mapStateToProps = ({
  spaceshipsReducer: { spaceships, hasPrevPage, hasNextPage, isFetching },
}: ISpaceshipsAwareState) => ({
  spaceships,
  hasPrevPage,
  hasNextPage,
  isFetching,
