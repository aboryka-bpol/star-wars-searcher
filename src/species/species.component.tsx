...
const Species = ({ fetchSpecies, species, hasPrevPage, hasNextPage, isFetching }: Props) => {
  ...
  return (
    <Container>
      <Searchbar placeholder='search for a species' onSearchChange={(search: string) => setDebouncedSearched(search)}/>
      <Grid
        container
        direction="row"
        justify="flex-start"
      ...
      >
        <Grid container item sm={12} md={6}>
          { !!species.length ? <ResourcesList resources={species}
                  hasPrev={hasPrevPage}
                  hasNext={hasNextPage}
                  page={queryParams.page}
                  onPageChange={changePage}
                  onResourceSelect={(species: ISpecies) => setSelectedResource(species)}
          /> : <ResourcesNotFound theme={theme}>No species found</ResourcesNotFound>}
        </Grid>
        <Grid container item sm={12} md={6}>
          { !!Object.keys(selectedResource).length && <SpeciesDetails {...selectedResource} /> }
        </Grid>
      </Grid>
    </Container>
  );
...
const mapStateToProps = (
  { speciesReducer: { species, hasPrevPage, hasNextPage, isFetching } }: ISpeciesAwareState
  ) => ({
    species,
    hasPrevPage,
    hasNextPage,
